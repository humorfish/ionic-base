...
import { DataService, ShopService } from '../../../provider';
...
export class ShopEvaluateComp {
  ...
  loaderMore(event: any) {
    ...
    this.dataService.getRatingList(this.shopId, this.ratingOffset).then(res => {
  ...
  getRatingList(ratingOffset: number, name?: string) {
    this.dataService.getRatingList(this.shopId, ratingOffset, name).then(res => {
      this.ratingList = this.shopService.ratingList;
    });
  }
